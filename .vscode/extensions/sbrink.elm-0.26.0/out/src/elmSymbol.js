...
function getDocumentSymbols(doc) {
    try {
        ...
        const moduleTypes = _.flatMap(parsedModule.types.map(t => {
            if (t.type === 'custom-type') {
                const customTypeSymbol = new vscode_1.DocumentSymbol(t.name, "", vscode.SymbolKind.Class, locationToRange(t.location), locationToRange(t.location));
                return t.constructors
                    .map(ctor => {
                    return new vscode_1.DocumentSymbol(ctor.name, "", vscode.SymbolKind.Constructor, locationToRange(ctor.location), locationToRange(ctor.location));
                })
                    .concat(customTypeSymbol);
            }
            else if (t.type === 'type-alias') {
                const typeAliasSymbol = new vscode_1.DocumentSymbol(t.name, "", vscode.SymbolKind.Class, locationToRange(t.location), locationToRange(t.location));
                return [typeAliasSymbol];
            }
            else {
                const _exhaustiveCheck = t;
        ...
        const moduleFunctions = parsedModule.function_declarations.map(f => {
            return new vscode_1.DocumentSymbol(f.name, "", vscode.SymbolKind.Variable, locationToRange(f.location), locationToRange(f.location));
        });
        const portAnnotations = parsedModule.port_annotations.map(p => {
            return new vscode_1.DocumentSymbol(p.name, "", vscode.SymbolKind.Interface, locationToRange(p.location), locationToRange(p.location));
        });
        const moduleDefinition = new vscode_1.DocumentSymbol(parsedModule.name, "", vscode.SymbolKind.Module, locationToRange(parsedModule.location), locationToRange(parsedModule.location));
        const allSymbols = _.concat(moduleDefinition, moduleTypes, moduleFunctions, portAnnotations);
        return allSymbols;
    }
    catch (error) {
